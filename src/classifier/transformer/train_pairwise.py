...
def load_triples_text():
    """Build (real, generated) text pairs from qrels.train.tsv x gen.train.tsv sharing a
    qid -- every real passage for a query paired with every generated passage for that same
    query. train_df already has the correct qid/pid/label from build_dataset.py."""
    ...
    for qid, reals in real_by_qid.items():
        for gt in gen_by_qid.get(qid, []):
            for rt in reals:
